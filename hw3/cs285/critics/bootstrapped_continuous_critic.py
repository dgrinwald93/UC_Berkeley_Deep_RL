...
class BootstrappedContinuousCritic(BaseCritic):
    ...
    def forward(self, ob):
        ...
        return self.sess.run(self.critic_prediction, feed_dict={
            self.sy_ob_no: ob
        })
    ...
    def update(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            ...
            if i % self.num_grad_steps_per_target_update == 0:
                ...
                v_s_ne = self.forward(next_ob_no)
                y_t = re_n + self.gamma * v_s_ne * (1 - terminal_n)

            # update this critic
            loss, _ = self.sess.run([self.critic_loss, self.critic_update_op],
                                    feed_dict={
                                        self.sy_ob_no: ob_no,
