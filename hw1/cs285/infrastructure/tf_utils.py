...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        for _ in range(n_layers):
            ...
            output_placeholder = tf.layers.dense(output_placeholder,
                                                 size,
                                                 activation) # HINT: use tf.layers.dense (specify <input>, <size>, activation=<?>)

        output_placeholder = tf.layers.dense(output_placeholder,
                                             output_size,
                                              output_activation) # HINT: use tf.layers.dense (specify <input>, <size>, activation=<?>)
